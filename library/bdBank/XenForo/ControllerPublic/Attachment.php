		...
		if (XenForo_Visitor::getUserId() > 0) {
			...
			if ($attachmentModel->canViewAttachment($attachment, $tempHash) AND $attachment['user_id'] != $userId) {
				...
				if ($point > 0) {
					...
					if (!$attachmentModel->isDownloaded($attachment, $userId)) {
						// IMPORTANT: now we will have to see if the user specified a price for this or not
						...
						$attachmentModel->markDownloaded($attachment, $userId);
					}
				}
			}
		}
